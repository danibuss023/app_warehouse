...
class ItemInPage extends StatefulWidget {
  const ItemInPage({super.key});

  @override
  State<ItemInPage> createState() => _ItemInPageState();
}

class _ItemInPageState extends State<ItemInPage> with TickerProviderStateMixin {
                          ...
                          child: SizedBox(
                            ...
                            child: ElevatedButton(
                              onPressed: isLoading ? null : () async {
                                ...
                                try {
                                  await _addStock(context, amount, note);
...
Future<void> _addStock(BuildContext context, int addAmount, String note) async {
  print('DEBUG: _addStock called with amount: $addAmount');
  ...
  try {
    ...
    try {
      await HistoryHelper.addHistoryEntry(
        ...
        newAmount: itemData['amount'] + addAmount,
        description: note.isNotEmpty ? note : 'Stock added via Item In page',
      );
    ...
    if (context.mounted) {
      ...
      await _showCenterNotification(context, addAmount);
    }
    
  } catch (e) {
    print('DEBUG: Error in _addStock: $e');
    if (context.mounted) {
      _showErrorNotification(context, 'Failed to add stock: $e');
    }
    rethrow;
  }
}
...
Future<void> _showCenterNotification(BuildContext context, int addAmount) async {
  print('DEBUG: _showCenterNotification called with amount: $addAmount');
                    ...
                    TweenAnimationBuilder<double>(
                      ...
                      builder: (context, value, child) {
                        return Opacity(
                          opacity: value,
                          child: Text(
                            'Stock "${itemData['name']}" added successfully',
                            style: const TextStyle(
                              color: Colors.white,
                              fontSize: 16,
                              fontWeight: FontWeight.w500,
                                ...
                                child: SizedBox(
                                  ...
                                  child: TextButton(
                                    onPressed: () {
                                      ...
                                      _undoLastTransaction(context, addAmount);
                                    },
                                    style: TextButton.styleFrom(
  ...
  Future<void> _undoLastTransaction(BuildContext context, int amount) async {
    try {
      ...
      await FirebaseFirestore.instance.runTransaction((transaction) async {
        ...
        if (newAmount < 0) {
          throw Exception('Cannot undo: would result in negative stock');
        }
        ...
        Future.delayed(Duration.zero, () async {
          try {
            await HistoryHelper.addHistoryEntry(
              ...
              newAmount: newAmount,
              description: 'Undo last Item In transaction',
            );
          } catch (e) {
